...
enum TypeAlerte {
  ...
  environnementale(
      'environnementale', 'Environnementale', Icons.eco, Colors.lightGreen),
...
class ModeleAlerte {
  ...
  ModeleAlerte.rappelRendezVous({
    ...
    String? telephone,
          ...
          description:
              'Rappel de votre rendez-vous avec $nomMedecin ($specialite)\n'
              '${adresse != null ? 'Adresse : $adresse\n' : ''}'
              '${telephone != null ? 'Téléphone : $telephone' : ''}',
          type: TypeAlerte.rendezvous,
          dateCreation: DateTime.now(),
          dateEcheance:
              dateRendezVous.subtract(const Duration(hours: 2)), // 2h avant
          niveauPriorite: 85,
  ...
  ModeleAlerte copiAvec({
    ...
    return ModeleAlerte(
      ...
      dateModification: DateTime.now(),
      niveauPriorite: severite?.priorite ??
          (priorite != null ? (priorite * 10).clamp(0, 100) : niveauPriorite),
  ...
  Map<String, dynamic> toJson() => versMap();

  /// Désérialise l'alerte depuis JSON
  static ModeleAlerte fromJson(Map<String, dynamic> json) => depuisMap(json);
  ...
  factory ModeleAlerte.depuisCarte(Map<String, dynamic> carte) {
    return ModeleAlerte.depuisAncien(
      ...
      recommandations: (carte['recommandations'] as String? ?? '')
          .split('|')
